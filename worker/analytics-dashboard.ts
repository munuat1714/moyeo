...
const EVENT_LABELS: Record<string, string> = {
  landing_view: '랜딩 방문', app_view: '정식 서비스 방문', demo_view: '데모 방문', landing_interest_yes: '관심 있음',
  landing_interest_not_yet: '아직 모름', landing_app_click: '서비스 시작 클릭', landing_demo_click: '데모 체험 클릭', room_created: '여행방 생성',
...
</style></head><body><main><header><div><span class="badge">MOHANG INSIGHTS</span><h1>익명 전환 통계</h1><p>한국시간 기준 · 새로고침하면 최신 합계를 불러옵니다.</p></div><p id="updated"></p></header><div class="toolbar"><select id="period" aria-label="조회 기간"><option value="7">최근 7일</option><option value="30" selected>최근 30일</option><option value="90">최근 90일</option><option value="all">전체</option></select><a id="csv" href="#">CSV 내려받기</a></div><div id="app" class="loading">통계를 불러오는 중이에요…</div></main><script>
...
fetch(location.pathname+'?format=json&period='+sel.value,{cache:'no-store'}).then(r=>{if(!r.ok)throw Error();return r.json()}).then(data=>{const counts=Object.fromEntries(data.totals.map(x=>[x.event_name,Number(x.session_count)]));const views=counts.landing_view||0,apps=counts.app_view||0,demos=counts.demo_view||0,rooms=counts.room_created||0,finals=counts.final_route_confirmed||0,rate=(a,b)=>b?Math.round(a/b*1000)/10:0,funnel=['landing_view','app_view','room_created','recommendations_viewed','final_route_confirmed'],max=Math.max(1,...funnel.map(k=>counts[k]||0)),grouped={},days={};data.campaigns.forEach(x=>{const key=[x.campaign_source||'(직접)',x.campaign_medium,x.campaign_name].filter(Boolean).join(' · ');grouped[key]=grouped[key]||{};grouped[key][x.event_name]=Number(x.session_count)});data.daily.forEach(x=>{days[x.event_date]=days[x.event_date]||{};days[x.event_date][x.event_name]=Number(x.session_count)});const dayMax=Math.max(1,...Object.values(days).flatMap(x=>[x.app_view||0,x.demo_view||0]));document.querySelector('#app').className='';document.querySelector('#app').innerHTML=\`<div class="cards"><div class="card"><small>랜딩 세션</small><b>\${number(views)}</b><em>선택 기간</em></div><div class="card"><small>정식 서비스</small><b>\${number(apps)}</b><em>데모 \${number(demos)}회</em></div><div class="card"><small>여행방 생성</small><b>\${number(rooms)}</b><em>서비스 대비 \${rate(rooms,apps)}%</em></div><div class="card"><small>최종 경로</small><b>\${number(finals)}</b><em>생성 대비 \${rate(finals,rooms)}%</em></div></div><section><h2>정식 서비스 전환 흐름</h2><p class="muted">데모 이벤트는 전환 흐름에서 제외합니다.</p><div class="funnel">\${funnel.map(k=>\`<div class="row"><span>\${esc(data.labels[k]||k)}</span><div class="track"><div class="fill" style="width:\${(counts[k]||0)/max*100}%"></div></div><strong>\${number(counts[k])}</strong></div>\`).join('')}</div></section><section><h2>날짜별 추이</h2><p class="muted">진한 파랑은 정식 서비스, 연한 파랑은 데모 세션입니다.</p><div class="daily">\${Object.entries(days).slice(-31).map(([day,c])=>\`<div class="daily-row"><span>\${day.slice(5)}</span><div><i style="width:\${(c.app_view||0)/dayMax*100}%"></i></div><div><i class="demo" style="width:\${(c.demo_view||0)/dayMax*100}%"></i></div></div>\`).join('')||'<div class="empty">아직 날짜별 데이터가 없습니다.</div>'}</div></section><section><h2>광고 캠페인별 결과</h2><p class="muted">utm_source, utm_medium, utm_campaign 기준입니다.</p><div class="campaigns">\${Object.keys(grouped).length?Object.entries(grouped).map(([name,c])=>\`<div class="campaign"><b>\${esc(name)}</b><strong>랜딩 \${number(c.landing_view)} · 서비스 \${number(c.app_view)} · 데모 \${number(c.demo_view)}</strong><span>생성 \${number(c.room_created)} · 최종 \${number(c.final_route_confirmed)}</span></div>\`).join(''):'<div class="empty">아직 UTM이 포함된 유입이 없습니다.</div>'}</div></section><p class="privacy">이름, 연락처, IP, 기기 식별자는 저장하지 않습니다. 날짜·이벤트·광고 UTM별 합계와 브라우저 세션당 최초 발생 여부만 집계합니다.</p>\`;document.querySelector('#updated').textContent=new Date().toLocaleString('ko-KR')+' 확인';}).catch(()=>{document.querySelector('#app').innerHTML='<p class="error">통계를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.</p>'});
</script></body></html>`
