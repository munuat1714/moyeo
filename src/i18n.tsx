...
export function LanguageSelect({ compact = false }: { compact?: boolean }) {
  ...
  return <div ref={root} className={`language-select ${compact ? 'compact' : ''}`}><button type="button" aria-haspopup="listbox" aria-expanded={open} aria-label="언어 선택" onClick={() => setOpen((current) => !current)}><Languages className="control-leading-icon" size={16} aria-hidden="true" /><span key={locale} data-no-translate>{labels[locale]}</span><ChevronDown className="control-chevron" size={15} aria-hidden="true" /></button>{open && <div className="language-menu" role="listbox" aria-label="언어 선택" data-no-translate>{(Object.keys(labels) as Locale[]).map((value) => <button type="button" role="option" aria-selected={locale === value} className={locale === value ? 'selected' : ''} key={value} onClick={() => { setLocale(value); setOpen(false) }}><span>{labels[value]}</span>{locale === value && <i aria-hidden="true">✓</i>}</button>)}</div>}</div>
}
